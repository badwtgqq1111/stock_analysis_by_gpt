...
- 财务数据、公告数据、另类数据尚未接入。
- `feature` 层已具备基础 schema、读写接口和首批 Qlib 风格因子产出能力，但仍缺少版本管理、缓存、横截面处理和验证流水线；`signal/trade` 仍无真实数据集和读写接口。
- 指数、ETF、杠反产品虽然在股票池过滤层有保留规则，但还没有独立的数据模型与资产分类体系。
- 数据质量校验已有基础巡检，但仍缺少停牌识别、节假日缺口识别、复权一致性校验和自动修复流水线。
- DuckDB 锁冲突和 WAL 异常已有 CLI 友好提示，但尚未做自动恢复或运维脚本。
...
- `signal/trade` 的真实读写闭环
- `feature` 层之上的真实因子产出、版本管理与缓存机制
- 企业行为官方校验源与本地复权回放
- 统一因子工厂接口
- 批量研究任务编排
- 事件驱动回测内核
...
| `P1` | `待开始` | 建立因子验证流水线：IC、RankIC、分组收益、换手率、衰减分析 | 因子评估报告 |
| `P1` | `待开始` | 建立机器学习因子挖掘流水线 | 支持 `LightGBM` 训练、验证和特征重要性输出 |
| `P1` | `待开始` | 建立符号回归因子挖掘流水线 | 支持 `gplearn` 自动生成候选表达式 |
| `P1` | `待开始` | 重构回测模块为事件驱动引擎 | 支持滑点、手续费、持仓与成交 |
| `P1` | `待开始` | 建立组合层：等权、风险预算、简单约束优化 | 从因子到权重的闭环 |
...
7. 建立 `Dash + Plotly` Web 工作台，先打通 K 线和回测结果展示。
8. 增加组合优化、实验记录、报告生成。
9. 再考虑实时数据、模拟交易和实盘接入。
