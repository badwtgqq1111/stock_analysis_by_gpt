...
class StockAnalyzer:
    ...
    @staticmethod
    def _compute_forward_metrics(data, horizons=(20, 40, 60), execution_delay=1):
        if data is None or data.empty:
            return pd.DataFrame(index=pd.Index([], name="date"))

        working = data[["Close", "Low", "High"]].copy()
        for horizon in horizons:
            # With execution delay: buy at T+delay, sell at T+delay+horizon
            # forward_return = close[T+delay+horizon] / close[T+delay] - 1
            future_close = working["Close"].shift(-(horizon + execution_delay))
            entry_close = working["Close"].shift(-execution_delay)
    ...
    def _compute_lightgbm_tactical_overlay(data):
        ...
        startup_frame = StockAnalyzer._compute_lightgbm_startup_feature_frame(data)
