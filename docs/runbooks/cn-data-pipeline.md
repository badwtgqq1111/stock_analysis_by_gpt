...
| # | 问题 | 实测证据 | 现行处理 |
|---|---|---|---|
| D1 | **128 个上证指数代码（`000001.SH` 上证指数 … `000148.SH`）曾被当成个股落库** | 修复前：OHLCV 库 395,008 行（2014-01-02→2026-09-09）、clean panel 47,232 行、训练窗口里每天 128 行（2.43%） | **已修复（2026-09-22）**：`is_cn_equity_code` 前缀白名单接入 universe/面板/训练三处 + `scripts/prune_cn_non_equity_codes.py` 清理元数据 + 重建面板（5349→5221 代码，指数行 0）+ 重训并做同折同日期 OOS A/B（RankIC 0.0993→0.0996，配对 p=0.20，**差异不显著**）；详见 P1_19 §1/§1.4 |
| D2 | **universe 断点**：2026-09-10 单日行数 −129（5,335→5,206） | 全库单日行数最大跳变（D1 的副作用） | **已消除（2026-09-22）**：重建后该日跳变 −1，全窗口最大跳变变为 +29（正常上市日） |
...
| D4 | 预检告警 `cn_ohlcv_rows_below_threshold` | = universe 代码集里有窗口内行数 <120 的代码（D3 的停牌 + 次新），不阻塞（`backtest_ready=true`） | D1 修完后指数项已消失；仍建议拆成具名原因（P1_19 §3） |
