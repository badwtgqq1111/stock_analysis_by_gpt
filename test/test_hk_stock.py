...
"""
测试港股历史数据获取
默认测试腾讯控股(00700)，覆盖日线 / 1min / 5min / 60min
"""

from data.ingest import HistoryDataFetcher
...
def test_hk_stock(stock_code="00700", data_source="akshare"):
    """
    测试港股数据获取
    :param stock_code: 港股代码，5位数字，比如腾讯控股是00700
    :param data_source: 数据源，可选 akshare/tencent，推荐 akshare
    """
    print(f"=== 测试 {stock_code} 港股数据获取，数据源：{data_source} ===")

    fetcher = HistoryDataFetcher(stock_code, data_source=data_source)
    success = True
...
if __name__ == "__main__":
    test_hk_stock(stock_code="00700", data_source="akshare")
